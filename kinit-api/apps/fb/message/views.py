            ...
            for event in data.get("entry", []):
                for messaging_event in event.get("messaging", []):
                    ...
                    if "instagram.com" in sender_id:
                        ...
                        # TODO 调用客服工作流回复
                        # await redis.xadd("ig_messages", {"data": json.dumps(event)})
                        ...
                        # TODO 调用客服工作流回复
                        coze_api = CozeApi(
                            COZE_TOKEN="pat_upP8xqabv9aUHSHDYu8xJN3S91BWBP8BG70IcUiGfP2bsgnpAOOjL1o41M1i6Dmu")
                        output, user_info = coze_api.send_foreign_trade_workflow(message, sender_id,
                                                                                 '+86 139 1030 5677',
                                                                                 'Ross Company Assistant')
                        if not output:
                            logger.error(f"[{platform.upper()}] Reply error: {output}")
                            continue
                        await send_fb_message(output, sender_id,
                                              auth_toke='EAAVL7FvW8ogBP0ebs0BQUbEQAE4DIbALtNbJY8vLscSe2BIyHdGKeZAREwXPHJtrGVeCzAqCKhyYWngMNmRnJEJF4bTPoVJiTZCwrpabYMKbrPOASMEeldIJMBclyJcbZBZAkw5vjlONUTOPHADanv2lDhwIeeD8CjhLwwDLsOPiPgqtAtJohtRN2eT55U3JbnCRKsxyNgZDZD')
                        # await redis.xadd("fb_messages", {"data": json.dumps(event)})
                    # print(f"[{platform.upper()}] New message: {message}")
